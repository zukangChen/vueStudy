    ...
    const initChat = () => {
        ...
        const options = {
           ...
           tooltip: {
                ...
                position :'right',
                formatter:  (data:any) => {
                    return `${data.name}<br>${data.marker} <span style="color:#fff">人数<span style="width:100px;text-align:right;display:inline-block;font-weight:600">${data.value}${opts.unit}</span></span><br><span style="color:#fff;padding-left:18px;">占比<span style="width:100px;text-align:right;display:inline-block;font-weight:600">${(data.value*100/sum).toFixed(2)}%</span></span>`;
                }
            },
            dataZoom: {
                start: 0,
             ...
             legend: {
                ...
                formatter:  (name:string) => {
                    ...
                    let zw = "";
                    for(let i = String(data1.value).length+name.length*1.2;i< 12;i++){
                        zw+="\t\t"
                    }
                    return ` ${name}{a|${zw}${data1.value}${opts.unit}} {b|${(data1.value*100/sum).toFixed(2)}%}\t\t\t\t`;
                },
                ...opts.legend
            },
            series: [{
